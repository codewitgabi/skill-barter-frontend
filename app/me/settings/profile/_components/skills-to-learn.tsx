...
function SkillsToLearn() {
  ...
  // Get skillsToLearn from user data
  const defaultSkills = useMemo((): SkillItem[] => {
    if (!user?.skillsToLearn) return [];
    return user.skillsToLearn
      .map((item) => ({
        name: item.name || "",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.status === "success" && response.data) {
        ...
        setUser(updatedUser);
        toast.success("Skills to learn updated successfully");
      } else {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Add skills you want to learn and your current level
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <div className="space-y-3">
            {skills.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground py-4 text-center border border-dashed rounded-lg">
                No skills added yet. Add skills you want to learn from others.
              </p>
            )}
            <div className="flex gap-2">
              <Input
                placeholder="e.g., Cooking, Spanish, Data Science"
                value={newSkill}
